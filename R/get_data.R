...
get_data <- function(species = NULL,
                     region = c("EBS", "NBS")[1],
                     district = NULL,
                     years = NULL,
                     channel = NULL){
  ...
  stock_stations <- data_haul %>%
    ...
    select(HAULJOIN, SURVEY_YEAR, STATION_ID, HAUL_TYPE, AREA_SWEPT, MID_LATITUDE, MID_LONGITUDE, DISTRICT_CODE, STRATUM_CODE, TOTAL_AREA) %>%
    rename(LATITUDE = MID_LATITUDE,
           LONGITUDE = MID_LONGITUDE)

  # Remove HT 17 if not RKC
  if(species != "Red King Crab"){
    stock_stations <- stock_stations %>% filter(!HAUL_TYPE == 17)
  }
  ...
  if(TRUE %in% (c("NORTH", "UNSTRAT") %in% strata)){
    ...
    data_crab2 <- data_crab2 %>%
      ...
      mutate(TOTAL_AREA = ifelse(TRUE %in% (c("NORTH", "UNSTRAT") %in% STRATUM_CODE), TOTAL_AREA_POS, TOTAL_AREA)) %>%
